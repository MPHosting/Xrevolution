...
const char * const BITCOIN_CONF_FILENAME = "xrev.conf";
const char * const BITCOIN_PID_FILENAME = "xrevd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "xrev" is a composite category enabling all Xrev-related debug output
            if(ptrCategory->count(string("xrev"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XrevCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XrevCore
    // Mac: ~/Library/Application Support/XrevCore
    // Unix: ~/.xrevcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XrevCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/XrevCore";
#else
    // Unix
    return pathRet / ".xrevcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty xrev.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override xrev.conf
